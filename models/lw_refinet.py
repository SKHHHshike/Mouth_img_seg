...
def lw_refinenet_mini(n_classes, input_height=360, input_width=480):
    if IMAGE_ORDERING == 'channels_first':
    ...
    conv1 = Conv2D(64, 3, activation='relu', padding='same', kernel_initializer='he_normal',
                   data_format=IMAGE_ORDERING)(conv1)
    f1 = conv1
    ...
    conv2 = Conv2D(128, 3, activation='relu', padding='same', kernel_initializer='he_normal',
                   data_format=IMAGE_ORDERING)(pool1)
    conv2 = Conv2D(128, 3, activation='relu', padding='same', kernel_initializer='he_normal',
                   data_format=IMAGE_ORDERING)(conv2)
    f2 = conv2
    ...
    conv3 = Conv2D(256, 3, activation='relu', padding='same', kernel_initializer='he_normal',
                   data_format=IMAGE_ORDERING)(pool2)
    conv3 = Conv2D(256, 3, activation='relu', padding='same', kernel_initializer='he_normal',
                   data_format=IMAGE_ORDERING)(conv3)
    f3 = conv3
    ...
    conv4 = Conv2D(512, 3, activation='relu', padding='same', kernel_initializer='he_normal',
                   data_format=IMAGE_ORDERING)(conv4)
...
def residual_conv_unit(filter, x):
    out1 = Conv2D(filter, 3, activation='relu', padding='same', kernel_initializer='he_normal',
                  data_format=IMAGE_ORDERING)(x)
    out2 = Conv2D(filter, 3, activation='relu', padding='same', kernel_initializer='he_normal',
                  data_format=IMAGE_ORDERING)(out1)
    return concatenate([x, out2], axis=MERGE_AXIS)


def fusion1(filter, x1, x2):
    x1 = Conv2D(filter, 1, activation='relu', padding='same', kernel_initializer='he_normal',
                data_format=IMAGE_ORDERING)(x1)
    x2 = Conv2D(filter, 1, activation='relu', padding='same', kernel_initializer='he_normal',
                data_format=IMAGE_ORDERING)(x2)
    return concatenate([x1, x2], axis=MERGE_AXIS)


def fusion2(filter, x1, x2, f1, f2):
    f1 = Conv2D(filter, 1, activation='relu', padding='same', kernel_initializer='he_normal',
                data_format=IMAGE_ORDERING)(f1)
    f2 = Conv2D(filter, 1, activation='relu', padding='same', kernel_initializer='he_normal',
                data_format=IMAGE_ORDERING)(f2)
...
def chained_residual_pooling(filter, x):
    x = Activation('relu')(x)
    sum0 = x
    # sum1 = MaxPooling2D(pool_size=(5, 5), data_format=IMAGE_ORDERING)(sum0)
    sum1 = Conv2D(filter, 1, activation='relu', padding='same', kernel_initializer='he_normal',
                  data_format=IMAGE_ORDERING)(sum0)

    # sum2 = MaxPooling2D(pool_size=(5, 5), data_format=IMAGE_ORDERING)(sum1)
    sum2 = Conv2D(filter, 1, activation='relu', padding='same', kernel_initializer='he_normal',
                  data_format=IMAGE_ORDERING)(sum1)

    # sum3 = MaxPooling2D(pool_size=(5, 5), data_format=IMAGE_ORDERING)(sum2)
    sum3 = Conv2D(filter, 1, activation='relu', padding='same', kernel_initializer='he_normal',
                  data_format=IMAGE_ORDERING)(sum2)
